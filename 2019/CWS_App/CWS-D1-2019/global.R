...
#stat_list = c("AB","BA","OBPct","SlgPct","OPS","TB","TA","ISO","SECA","RC","RC27","OPG")
...
conf_df <- master_ncaa_team_lu %>% filter(year==2019) %>% select(school,conference) 
team_names <- team_hit %>% select(team_name) %>% distinct()

conf_list <- conf_df %>% inner_join(.,team_names,by=c("school"="team_name")) %>% 
  select(conference) %>% distinct() %>% pull() %>% sort()
team_list <- team_names %>% pull() %>% sort()

top_hit <- drop_read_csv("NCAA_D1_BShiny/summary_hitting_data.tsv",sep="\t")
top_hit <- top_hit %>% mutate(
  X1B = H - (X2B + X3B + HR)
...
summary_pitching = drop_read_csv("NCAA_D1_BShiny/summary_team_pitching.csv")
