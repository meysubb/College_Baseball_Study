...
yr_2017 <- master_ncaa_team_lu %>% filter(year==2017)
### Final Team/Conference and Win vs Pyth win Record Data-Frame. Make sure to save
team_name_conf <- overall_teams %>% inner_join(.,yr_2017,by=c("team_name"="school"))

team_name_conf_final <- team_name_conf %>% select(team_name,pyth_w_pct,w_pct,conference)
